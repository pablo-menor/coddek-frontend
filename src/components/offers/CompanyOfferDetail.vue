<template>
    ...
    <section class="main-details">
      ...
      <div class="details-mobile">
        ...
        <span class="detail-mobile">
          ...
          Experiencia:
          {{ offer.experienceRequired || "no requerida" }}
        </span>
        <span class="detail-mobile">
          <svg
            width="14"
          ...
          {{ offer.location || "no especificado" }}
        </span>
      </div>
    </section>
    <section class="description">
      <p class="description-text-mobile">{{ offer.description }}</p>
      <button class="challange-details" @click="changeInscribed()">
        Ver inscritos
      </button>
    </section>
  ...
  <div
    v-show="showInscribed == true"
    @close="showInscribed = false"
    class="container-offer-inscribed-devs"
  >
    <div class="offer-inscribed-devs">
      <span class="applied-title">Inscritos</span>
      <div class="dev-list">
        <div class="applied-dev" v-for="(ele, i) in devs" :key="i">
          <div class="data-dev">
            <h3>{{ ele.username }}</h3>
            <span>{{ ele.email }}</span>
          </div>
          <div class="download-options">
            <span
              class="btn-solution btn-option"
              @click="showSolution(this.offer.devs[i].solution)"
              >Solución</span
            >
            <span class="btn-cv btn-option" @click="showCV(ele.cv[0].fileName)"
              >CV</span
            >
            <span
              class="btn-select-off btn-option"
              v-show="this.offer.devs[i].selected == false"
              @click="this.offer.devs[i].selected = true"
              ><i class="fa-solid fa-user-plus"></i
            ></span>
            <span
              class="btn-select-on btn-option"
              v-show="this.offer.devs[i].selected == true"
              @click="this.offer.devs[i].selected = false"
    ...
    <i class="fa-solid fa-x cancel-mobile" @click="closeAppliedDevs()"></i>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getAppliedDevs() {
      this.appliedDevs = this.offer.devs;
      for (let dev of this.appliedDevs) {
        devService.getById(dev.userId).then((dev) => {
          console.log();
          this.devs.push(dev);
        });
      }
    ...
    showSolution(solution) {
      window.open(`http://localhost:3008/${solution}`);
    },
    showCV(cv) {
      window.open(`http://localhost:3008/${cv}`);
    },
  },
};
</script>
...
.container-offer-inscribed-devs {
  border-radius: 5px;
  width: 85%;
  max-width: 490px;
  height: 97vh;
